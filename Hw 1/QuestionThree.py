...
std_data = standardize(Matrix)

k_values = range(1, Matrix.shape[0] + 1)
video_frames = []
...
for i, k in enumerate(k_values):
    ...
    reconstructed_arr = np.sum(w * z[:, np.newaxis], axis=0) + np.mean(Matrix, axis=0)
